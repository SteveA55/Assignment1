...
async function listBooks(filters?: Filter[]): Promise<Book[]> {
  ...
  const result = await fetch('http://localhost:8080/api/books/list', { body: JSON.stringify(filters ?? []), method: 'POST' })
...
async function lookupBookById(book: BookID): Promise<Book> {
  const result = await fetch(`http://localhost:8080/api/books/${book}`)
  if (result.ok) {
    return await result.json() as Book
  } else {
...
async function placeBooksOnShelf(bookId: BookID, numberOfBooks: number, shelf: ShelfId): Promise<void> {
  const result = await fetch(`http://localhost:8080/api/warehouse/${bookId}/${shelf}/${numberOfBooks}`, { method: 'put' })
  if (!result.ok) {
...
async function findBookOnShelf(book: BookID): Promise<Array<{ shelf: ShelfId, count: number }>> {
  const result = await fetch(`http://localhost:8080/api/warehouse/${book}`)
  if (result.ok) {
...
async function fulfilOrder(order: OrderId, booksFulfilled: Array<{ book: BookID, shelf: ShelfId, numberOfBooks: number }>): Promise<void> {
  const result = await fetch(`http://localhost:8080/api/fulfil/${order}`, {
    method: 'put',
    headers: { 'Content-Type': 'application/json' },
